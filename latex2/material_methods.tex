...
Additionally to the rods which would be used to assess the distortion, a number of vitamin-e pills were attached to the frame (see figure \ref{fig:pills}).
...
After loading the image data, the script sets a slice chosen by the user as reference slice (\textbf{1.}).
...
In the future more sophisticated approach should be implemented where the script chooses a suitable value based on the image modality and the image size.
If the difference exceeds a this value (\textbf{5b.}), the current slice will be marked as irregular and consequently won't be used to calculate its DC or COM.

\subsubsection{Irregular slices and their COM and DC values}
Instead of actually calculating these and the values describing the location shift between MRI and CT scans, they will all be set to "$-1$".
...
\begin{figure}[p]
    ...
    \node [blockb, right of=3, text width=8em, node distance=8cm] (repeat!) {Repeat comparison with next slice\\ \texttt{i $\rightarrow$ i + 1}};
...
The implemented function compares a binary image (input A) to a circle (reference B).
...
\begin{figure}[p]
    ...
    \node [block, below of=init-guess] (getDC) {\textbf{3.} calculate DC for\\upper and lower half\\of current range:\\\texttt{DC$_{up}$ = DC @ (guess+max)/2\\ DC$_{lo}$ = DC @ (guess+min)/2}};
    \node [blockb, right of=getDC, text width=8em, node distance=6cm] (repeat!) {Repeat\\ iteration step};
    \node [decisiong, below of=getDC, node distance=3cm] (bigger) {\textbf{4.} which one is bigger?};
    ...
    \node [block, below of=bigger, yshift=-2.5cm] (next-guess) {\textbf{6.} New current guess at middle of new range:\\\texttt{guess =  (min+max)/2}};
    \node [decisiong, below of=next-guess, node distance=2.5cm] (repeat) {\textbf{7.} Repeat?};
    \node [blockb, text width=20em, below of=repeat] (fin) {\textbf{8.} End of iteration:\\\texttt{return best DC\\ return best guess (as threshold)}};
    % Draw edges
