...
\begin{itemize}
 ...
 \item find slices which show irregularities and mark those as such
 ...
 \item measure the local distortion
  ...
  \subitem dice coefficient ( \textbf{``DC''}) describing the roundness
...
It is the user's responsibility to find one that is free from any such objects and set it tp be the reference slice.
Ideally, it is located close to the iso-centre and has a brightness that is representative for the whole image.

To decide whether a slice is 'irregular', it's average brightness will be compared to the reference slice's.
If the difference exceeds a certain value, the slice will be marked as irregular and consequently not used to calculate its DC or COM.
Instead all associated values will be set to '$-1$'.

\subsubsection{Measuring distortion}

Two phenomena were chosen to reflect the amount of distortion occurring in MRI scans.
The distance which the rod appears to be shifted in the MRI scan compared to the CT scan in a particular slice is what will be referred to as \textbf{``warp''}.
The rod's deformation (deviation from circular profile) in a particular slice will be described using the dice-coefficient \textbf{``DC''} (also known as Sorensen-Index).
\\
