...
\begin{table}[!tbh]
\centering
\caption{MRI scanner and used protocol.}
\begin{tabular}{r|l}
system			& MRI				\\
\toprule
...
\item \textit{3D Slicer}: (Versions: Slicer-4.5.0-1-linux-amd64, Slicer-4.6.2-win-amd64) a "free and open source software package for visualisation and medical image computing". \cite{Fedorov2012, Kikinis2013, } was used to crop and resample images, quickly read values and visualise the results.
\end{itemize}
...
\item Next, as the MR image had a lower resolution than the CT scan, the MRI scan was resampled.
...
\textit{SimpleITK} is a object-oriented "C\texttt{++} library with wrappers for Python, Java, CSharp, R, Tcl and Ruby". \cite{Lowekamp2013, SimpleITK, SimpleITK_started} Its versatility is one of the reasons why this approach was favoured.
...
An alternative way to handle DICOM data in Python would be Pydicom. \cite{Mason2011, Pydicom, Kyriakou-Pydicom-VTK} 
...
The implementation as python function is based on the open source python package "Medpy". \cite{MedPy} A part of its module called "metric" was adapted \cite{MedPy_dc-doc}.\\
...
The dice coefficient or Sorensen indexis defined as:
\begin{align}
\label{eq:DC}
...
Alternatively, the DC fo the MRI scan can be calculated with the reference circle B placed in the COM of the corresponding CT image.
...
\begin{figure}[!htb]
	...
	\end{tikzpicture}
	\caption[Flowchart: \texttt{DC} calculation for an MRI scan yielding a value of $0.9454$.]{Flowchart: \texttt{DC} calculation for an MRI scan (rod \#5 @ slice 200) yielding a value of $0.9454$}
	\label{flo:DC}
\end{figure}
