...
% \item de-noise the image data
 ...
 \item export a rod shaped scan where the pixel values reflect the distortion occurring in each slice instead of their brightness as a ".mha" file (useful for visualisation, see figure \ref{fig:colour-map} \todo{ref to colour map}).
\end{itemize}
...
The \texttt{COM} is calculated using those pixels which exceed the  \texttt{threshold}, but weighted with their actual brightness (not using the binary image)

This way the script is supposed to place the reference circle B in the centre of the input image A.
The position of the circle's centre and its radius highly influence the outcome. (See chapter \ref{sec:discussion_DC} for a more thorough discussion.)
...
\begin{description}
 ...
 \item[2] finding a \texttt{COM} resulting in a good \texttt{DC}
\end{description}
...
%    % Draw edges
	...
	\end{tikzpicture}
	\caption{Method to find \texttt{threshold} for \texttt{COM} calculation based on real rod size}
	\label{flo:threshold_simple}
\end{figure}
...
\begin{figure}[!htp]
    ...
    \node [blockb, below of=img, text width=10em] (img_t) {\texttt{original image}};
    \node [blockb, below of=img_t, xshift=-4cm, text width=16em] (binary_t) {\texttt{binary input image A}:\\pixels where original image is darker than \texttt{threhold} are set to \texttt{0}\\ the others are set to \texttt{1}};
    \node [blockb, below of=img_t, xshift=4cm, text width=16em] (circle_t) {\texttt{reference circle B} with centre at \texttt{COM} of \texttt{original image}};
    ...
    \node [blockb, below of=img_t, yshift=-8cm, text width=10em] (overlap) {calculate the overlap: \\ \large \texttt{DC} = \Large $\frac{2 |A \, \cap \, B|}{|A| + |B|}$};
    % Draw edges
...
\begin{figure}[p]
    ...
    \node [block, below of=init-guess] (getDC) {\textbf{(3)} calculate \\\texttt{COM} coordinates and \texttt{DC} for\\upper and lower half\\of current range:\\\texttt{DC$_{up}$ = DC @ (guess+max)/2\\ DC$_{lo}$ = DC @ (guess+min)/2}};
...
\todo{explain more}
Comparing the two \texttt{DC}s will decide which guess is closer to representing the rod better (\textbf{4}).
If the lower number of pixels yields a better \texttt{DC} (\textbf{5a}), the upper half of the range will be neglected in the next iteration or in other words, the new upper limit takes the value of the former guess ($50\%$).
If the higher percentage yields a better \texttt{DC} (\textbf{5b}), the lower half of the range will be neglected in the next iteration, the new lower limit takes the value of the former guess ($50\%$).
At the end of the iteration (\textbf{6}), the percentage resulting in the higher \texttt{DC} is saved as the new current guess.
After this first iteration (\textbf{7}) the script can either repeat steps 3-6 or end it by returning the best guess and its \texttt{DC} (\textbf{8}).\\
...
If, for example, the \texttt{DC} for $25\%$ was higher than for $75\%$, the next guess will be $25\%$, because the new range goes from $0\%$ to $50\%$.
...
If, on the other hand, the \texttt{DC} for $75\%$ was higher, the next guess will then be $75\%$, because the new range goes from $50\%$ to $75\%$.
In that case, \texttt{DC}s for $62,5\%$ and $82,5\%$ will be compared.

The iteration continues until further steps yield no better \texttt{DC} or a set number of steps has been performed.
...
\centering
  ...
  \centering
    \includegraphics[scale=0.7]{python/ph2/COM_iter/ph2_CT_x100_COM_iter-far.png}
    \caption{full iteration process}
    \label{fig:CT_x100_iteration}
  \end{subfigure}
  \begin{subfigure}[b]{1\textwidth}
  \centering
    \includegraphics[scale=0.7]{python/ph2/COM_iter/ph2_CT_x100_COM_iter.png}
     \caption{close up}
