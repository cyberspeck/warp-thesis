...
% \item de-noise the image data
 ...
 \item measure the local distortion described by the
  \subitem location shift (refered to as \textbf{"warp"})
  \subitem dice coefficient (the \textbf{"DC"} refers to an object's roundness)
...
\todo{add figure with colour coded map}
...
If the difference exceeds a certain value (\textbf{5b.}), the current slice will be marked as irregular and consequently won't be used to calculate its DC or COM.
...
Instead of calculating meaningless DC, COM, and warp numbers, they will all be set to "$-1$".
