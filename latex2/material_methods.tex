...
To measure distortion the scanners must image a rigid object with known dimensions. Such a so called 'phantom' is often made from plastics containing a liquid. The AKH's design relies on an array of replaceable fillable plastic rods.
...
``x9'' is a resolution resulting in 1 pixel being split in 9 smaller pixels, ``x100'' in 100, and so on.
For better visibility images are printed with inverted colours. Dark pixels have a high density/intensity value, white pixels are equivalent to air (low density/intensity).
