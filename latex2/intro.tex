...
%\todo{Some introductory intorduction is needed. This should be some half a page of bla bla about the interdisciplinary of the current and future medicine where the success and development is dependent on the work of both medical personnel such as doctors nurses, but also biologists, chemists, physicists and engineers. That the problems we are putting ourselves in front of are becoming more and more complex and need the interdisciplinary expertise. This means that trying to answer questions that seem to be easy and sometimes are so, if you are satisfied with a more or less precise answer, become very complex when we try to improve the precision and go towards targeted therapies which are tailored very precisely to the needs of certain patient to improve his or her quality of life, chance of survival and in the long run also the cost of used resources. And that this work is a small contribution to this development.}
...
\todo{Please work on that a bit more. As a guideline use the following chapters from the ref [41]:
...
14.5.5.
14.9
14.12 introductory part (till the figure).

It does not need to be long, but what you have by now is too simplistic.}
...
In the case of an applied external magnetic field (this static field is often called B0), the spins will either align parallel (pointing in the same direction) or anti-parallel (opposite direction) to this field, minimising their energy.
...
MR imaging gives practically unlimited possibilities in terms of selective imaging and is mostly limited to imaging time and technical characteristics of the scanner.
...
However, for Radio Therapy Treatment Planning this might not be enough and it is necessary to additionally monitor the distortion and, if necessary, take additional corrective measures. 
...
\caption{On the left is the original image, on the right the automatically corrected version of a MR image(inverted colours)}
...
Despite the fact this type of MR imaging was not used for this thesis, it will be mentioned here as an example of the vast range of measurements possible with MRI techniques.
\\
...
During treatment planning, 3-D models of the patient are used to define targets (regions where the dose should go) and organs at risk (where the dose should not go).
This ensures that vulnerable organs are spared from radiation while making sure the tumours receive sufficient dose.
Moreover, methods to quantify the amount of radiation that different body parts absorbed are needed, because the actual treatment might be different to the plan.\\
...
During treatment, the smaller parts of the total dose are administered from many different locations.
The sum of those individual treatments result in the required dose distribution.
Figure \ref{plan} shows an example of two differently calculated treatment plans.
...
Unfortunately, radiation interacts with all cells it is passing through until it is fully absorbed.
...
Either the source is moved close to the target area using applicators (temporary treatment) or implanted permanently.
...
These rice seed sized implants can remain in the body without causing any harm.
...
For some, a non-invasive method (EBRT) is the only option.\\

Other types of ionising radiation are also, but less common.
Electrons and low energy X-rays are favoured for some superficial tumours, rare methods using neutrons and even muons also exist.
Charged particle therapy is on the rise, but still not as common as X-rays.
...
\begin{figure}[!tbh]
	...
	\caption{Example of Radio Therapy Plan: colored areas represent different values of radiation administered during treatment (image source: \cite{piotr})}
	\label{fig:plan}
\end{figure}
...
\todo{Generally the paragraph is carrying the right message but is should be better organised. Some of that you already mentioned before in the chapter about CT. Make use of this and put the two arguments in more elegant words.}

Until recently, radiotherapy treatment planning (RTP) relied almost entirely on Computer Tomography (CT).
There are two main reasons for this:

Firstly, CT uses low energy x-rays to create a 3D image of the patient.
The luminosity value (brightness) assigned to each voxel corresponds to the local radiodensity recorded in Hounsfield units ($HU$).
Materials with a higher radiodensity (e.g. bones) absorb more x-ray photons than those with less (e.g. water, brain-matter).
Calculating the electron density using data obtained with CT is an straightforward task and used widely for RTP. \cite{Constantinou2012, Schneider1996}
...
MR images also record luminosity values, but they do not correspond to $HU$ (which is directly related to radiodensity, measured by CT).
...
\todo{Write a snetence that there are algorithms supporting non rigid registration but that there is still space for a lot of imporvement. Therefore typically rigid registration is performed locally to support taget and OAR definition.}
Therefore MRI-only radiation therapy protocols are being developed:
A way to that is when MRI data is used to create a Pseudo-CT, which contains information about electron density.
...
\todo{Add that in that way one can also save time and money by only performing one imaging - MRI insetead of CT and MRI. And also there is benefit of not adding dose and the quantitative fictional parameters from DW-MRI or DCE-MRI support response prediction and assessment.}
