...
As the reparation process needs some time \todo{SAME FOR ALL TISSUES???}, all unwanted changes occurring less frequently than one reparation cycle are less probable to be implemented for good, than those taking place more frequently. This is the reason why cells in tissues and organs that divide more frequently (i.e. gonads) are more prone to mutations than those reproducing more slowly (i.e. bones).
...
Low field MRI scanners are also typically characterised by less distorted images.
\todo{(What is this supposed to mean?)Although the image distortion depends on all magnetic fields applied to the imaged subject during the acquisition (B0, B1 and gradients) and some of their components might be predicted and corrected for, they are all dependent on the initial strength of the B0 field.}
...
\section{External Beam Radiation Therapy Planning}
\label{sec:planning}
...
\begin{figure}[!tbh]
	\centering
	\includegraphics[width=0.6\textwidth]{abdoplan.jpg}
	\caption{Example of Radio Therapy Plan: colored areas represent different values of radiation administered during treatment \todo{use own image!}}
	\label{fig:plan}
\end{figure}
