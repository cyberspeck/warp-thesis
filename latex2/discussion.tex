...
Slices 303-310 in table \ref{tab:spit-out-5} are where the maximum brightness dropped significantly due to the absence of liquid (see figure \ref{fig:ph2_MR_x100_brightness}).
...
Consequently, warp and the DC for MRI were set to '-1'.
...
As slices with values of '-1' influence the average dramatically, the chosen threshold is low enough (~339; see appendix) to include some of the relatively dark pixels in slices 303-310 \\
...
Because the bubble was located on top of the liquid, the COM shift is most pronounced in the y direction (up-down) as shown by figure \ref{fig:ph2_warpXY_x100}.
...
Interestingly, the y shift shows is oriented to the opposite direction in front and behind the centre (see figure \ref{fig:ph3_warpXY_x100}).
This might be a local feature of the distortion and cannot be put in relation, because no distortion map of the entire FOV has been created yet.

As rod \#17 does not contain any air bubbles, the brightness plotted in figure \ref{fig:ph3_MR_x100_brightness} shows no sudden drops.
However, the left hand side has a drastically lower signal strength than the right.
The rod was not placed at in the same place as rod \#5, which does not show such a gradient.
One reason for the low signal strength might be the short duration of the MRI sequence used for this scan resulting in low visibility in outer regions.
Another possible explanation is that the imaged liquid separated in parts with different density while the rod was stored upright.
More experiments would be needed to find the cause.

In the region beyond -330, there is a drop of the DC for the CT.
Investigations revealed that it is caused by sticky tape attached to the rod to hold it in place during transport from the CT scanner to the MRI scanner.
...
Consequently, in table \ref{tab:spit-out-5} and \ref{tab:spit-out-17}, the values of the COM shift ($warp_x$, $warp_y$, $warpM$); the DC for CT using the CT-centroid ($DC_{CT}$); and the DC for MRI using the CT-centroid ($DC_{MR(CT-COM)}$) are all set to '-1'.
...
Figures \ref{fig:ph2_warpMagnitude_x1-100} and \ref{fig:ph3_v2_warpMagnitude_x1-100} depict the measured $warpM^*$ of rod \#5 and \#17 for x1 and x100 resample rates.
The calculated DC is shown in figures \ref{fig:ph2_DC_x1} and \ref{fig:ph3_DC_x1}.
Figures \ref{fig:ph2_CT_DC-15iter}, \ref{fig:ph2_MR_DC-15iter}, \ref{fig:ph2_MR_CT-COM_DC-15iter}, \ref{fig:ph3_CT_DC-15iter}, \ref{fig:ph3_MR_DC-15iter} and \ref{fig:ph3_MR_CT-COM_DC-15iter} visualise how the maximum DC values increased with higher resample rates.
The location shift calculated with x1 and x100 resample rate are relatively close.
...
\end{tabular}
\caption{Change in calculated distortion due to resample rate; rod \#5}
...
\begin{figure}[!bth]
  ...
  \caption{Rod \#5: CT average DC of varied radii \& resolutions; simple method}
...
\begin{figure}[!tbh]
  \centering
    ...
    \caption{Rod \#5: MRI average DC of varied radii \& resolutions; simple method}
...
\begin{figure}[!tbh]
    ...
    \caption{Rod \#5: MRI average DC of varied radii \& resolutions (using CT-COM); simple method}
...
\end{tabular}
\caption{Change in calculated distortion due to resample rate; rod \#17}
\label{tab:Delta-resample_17}
\end{table}

\begin{figure}[!tbh]
    \centering
    \includegraphics[scale=0.6]{../fig/python/ph3_v2/warp/ph3_MR_v2_x1-100_warpMagnitude_iter.png}
    \caption{Rod \#17: warp Magnitude [$mm$] (iteration method), CT-MRI x1 and x100}
...
\begin{figure}[!tbh]
  \centering
  \includegraphics[scale=0.6]{python/ph3_v2/dice/ph3_CT_DC-15radii_simple_close.png}
  \caption{Rod \#5: CT average DC of varied radii \& resolutions; simple method}
  \label{fig:ph3_CT_DC-15iter}
\end{figure}

\begin{figure}[!tbh]
  \centering
    \includegraphics[scale=0.6]{python/ph3_v2/dice/ph3_MR_v2_DC-15radii_simple.png}
    \caption{Rod \#5: MRI average DC of varied radii \& resolutions; simple method}
    \label{fig:ph3_MR_DC-15iter}
\end{figure}

\begin{figure}[!tbh]
      \centering
    \includegraphics[scale=0.6]{python/ph3_v2/dice/ph3_MR_v2_DC-15radii_CT_COM_simple_close.png}
    \caption{Rod \#5: MRI average DC of varied radii \& resolutions (using CT-COM); simple method}
    \label{fig:ph3_MR_CT-COM_DC-15iter}
\end{figure}
