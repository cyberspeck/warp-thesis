...
This was necessary, because the MR scanner needs sufficient signal for shimming prior to the start of imaging.
...
\subsection{Visibility on CT/MR scans}
...
On MR scans most liquids had a mean and a max brightness value above $1000$ (see table \ref{tab:visibility}).\\
Only \textbf{\#1}, \textbf{\#9}, \textbf{\#10}, \& \textbf{\#16} resulted in significantly less signal (below < $1000$).


\begin{table}[!htb]
\centering
\caption{Visibility of liquids on MR scans.}
\begin{tabular}{@{}l|lllll@{}}
\toprule
No. & Min  & Max  & Mean   & Median & $\sigma$ \\
\toprule
...
\footnotetext{liquid exhibited brown colouring.}
\clearpage
...
This situation provided data containing an irregularity (bubble) and was therefore well suited for testing some of the software tool's capabilities and limitations.
 ...
 \begin{table}[p]
    ...
    \rotatebox{90}{
      \begin{minipage}{\textheight}\footnotesize
      	\caption[Rod \#5; x100: script generated data.]{Rod \#5; x100: script generated data; dist and $warp$ in [$mm$].}
        \centering
 \begin{tabular}{rr||lll|lll||lll|lll}
 slice	& dist & $warp_x$ & $warp_y$ & $warpM$ & $DC_{CT}$ & $DC_{MR}$ & $DC_{MR(CT-COM)}$ & $warp_x^*$ & $warp_y^*$ & $warpM^*$ & $DC^*_{CT}$	& $DC^*_{MR}$ & $DC^*_{MR(CT-COM)}$\\ \toprule
...
\begin{figure}[!tbh]
    ...
    \caption[Rod \#5; x100: CT brightness along z-axis.]{Rod \#5; x100: CT brightness of all pixels in each slice along z-axis; the plastic pane fills the area from $-10mm$ to $10mm$.}
...
\begin{figure}[!tbh]
    ...
    \caption[Rod \#5; x100: MRI brightness along z-axis.]{Rod \#5; x100: MRI brightness of all pixels in each slice along z-axis; drop of max brightness at around $110mm$ to $135mm$ is due to air bubble.}
...
\begin{figure}[!bth]
  ...
  \caption[Rod \#5; x100: warp XY (iteration method).]{Rod \#5; x100: warp XY [$mm$] (iteration method); plastic pane present from $-10mm$ to $10mm$, bubble at about $110mm$ to $135mm$.}
...
\begin{figure}[!tbh]
    ...
    \caption[Rod \#5; x100: warp Magnitude (iteration method).]{Rod \#5; x100: warp Magnitude [$mm$] (iteration method); plastic pane present from $-10mm$ to $10mm$, bubble at about $110mm$ to $135mm$.}
...
\begin{figure}[!bth]
    ...
    \caption[Rod \#5; x100: DC (iteration method) for CT \& MRI \& MRI (using CT COM).]{Rod \#5; x100: DC (iteration method) for CT \& MRI \& MRI (using CT COM); plastic pane at $-10mm$ to $10mm$, bubble at about $110mm$ to $135mm$.}
...
\begin{figure}[!tbp]
  ...
  \end{subfigure}
  \caption[Rod \#5; x100: images showing the location of MRI/CT centroids at various slices along z-axis.]{Rod \#5; x100: images showing the location of MRI/CT centroids at various slices along z-axis (true colours). The dark blue dot represents the MRI centroid, the bright red dot represents the CT centroid.
...
After deciding which of the possible liquids might be a suitable choice for the future use of the phantom, another set of MRI and CT scans was taken.
...
\begin{table}[p]
   ...
   \rotatebox{90}{
     \begin{minipage}{\textheight}\footnotesize
       \centering
       \caption[Rod \#16; x100: script generated data.]{Rod \#16; x100: script generated data; dist and $warp$ in [$mm$].}
\begin{tabular}{rr||lll|lll||lll|lll}
slice	& dist & $warp_x$ & $warp_y$ & $warpM$ & $DC_{CT}$ & $DC_{MR}$ & $DC_{MR(CT-COM)}$ & $warp_x^*$ & $warp_y^*$ & $warpM^*$ & $DC^*_{CT}$	& $DC^*_{MR}$ & $DC^*_{MR(CT-COM)}$\\ \toprule
...
\begin{figure}[!thb]
    ...
    \caption[Rod \#16; x100: CT brightness along z-axis.]{Rod \#16; x100: CT brightness of all pixels in each slice along z-axis; the plastic pane fills the area from $-10mm$ to $10mm$.}
...
\begin{figure}[!tbh]
    ...
    \caption[Rod \#16; x100: MRI brightness along z-axis]{Rod \#16; x100: MRI brightness of all pixels in each slice along z-axis.}
...
\begin{figure}[!hbt]
  ...
  \caption[Rod \#16; x100: warp XY (iteration method).]{Rod \#16; x100: warp XY [$mm$] (iteration method); plastic pane fills region from $-8mm$ to $12mm$.}
...
\begin{figure}[!htb]
    ...
    \caption[Rod \#16; x100: warp Magnitude (iteration method.)]{Rod \#16; x100: warp Magnitude (iteration method); plastic pane fills region from $-8mm$ to $12mm$.}
...
\begin{figure}[!htb]
     ...
     \caption[Rod \#16; x100: DC (iteration method) for CT \& MRI \& MRI (using CT COM).]{Rod \#16; x100: DC (iteration method) for CT \& MRI \& MRI (using CT COM); plastic pane fills region from $-8mm$ to $12mm$.}
